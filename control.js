...
db.serialize(() => {
  ...
  db.run(
    `CREATE TABLE IF NOT EXISTS channel(id text unique, code text, level int, hp int, mp int)`,
  );
    ...
    return new Promise((res) => {
      db.serialize(() => {
        if (typeof id != 'string') {
          let re = [];
          for (let key in id)
            re.push(`${key} = ${typeof id[key] == 'string' ? `"${id[key]}"` : id[key]}`);
          id = re.join(' and ');
        } else {
          id = `id = "${id}"`;
        }
        if (typeof content != 'string') {
          ...
          for (let key in content)
            re2.push(
              `${key} = ${typeof content[key] == 'string' ? `"${content[key]}"` : content[key]}`,
            );
          content = re2.join(', ');
        }
