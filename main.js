...
const embed = (_) => new Discord.EmbedBuilder(_);

const EXP_MULTIPLY = 2;

const intent = Discord.GatewayIntentBits;

const client = new Discord.Client({
  intents: [intent.Guilds, intent.MessageContent, intent.GuildMembers, intent.GuildMessages],
});

const commanding = new Discord.Collection();

const getItem = async (table = 'item', user, item, amount) => {
  return new Promise(async (res) => {
    const check = await control.read(
      table,
      {
        id: user,
        item: item,
    ...
    if (check) {
      ...
      await control.replace(table, user, {
        item: item,
        amount: amount,
      });
      res(() => {});
...
const selectSkill = (arr, nores = null) => {
  let ski = null;
  arr.forEach((s) => {
    if (ski) return;
  ...
  return ski || (nores ? null : selectSkill(arr));
};
...
client.on('ready', () => {
  ...
  client.user.setPresence({
    activities: [
      {
        name: `${config.prefix}help｜SAMPLE`,
      },
    ],
  });
});
...
const result = async (message, channel, content) => {
  let inbattle = await control.read(
    'inbattle',
    {
  ...
  let exp = channel.level * EXP_MULTIPLY;
  let resultlog = [`参加者は ${exp} の経験値を獲得した！`];
  let itemlog = [];

  if (inbattle) {
    for (let i = 0; i < inbattle.length; i++) {
      let player = await control.read('player', inbattle[i].id);
      let user = await client.users.fetch(inbattle[i].id);
      ...
      if (Math.floor(Math.sqrt(player.exp + exp)) > Math.floor(Math.sqrt(player.exp))) {
        resultlog.push(
          `${user}はレベルアップした！\`Lv.${Math.floor(Math.sqrt(player.exp))} -> Lv.${Math.floor(Math.sqrt(player.exp + exp))}\``,
        );
      }
      player.exp += exp;
      ...
      for (let v = 0; v < enemy.drop.length; v++) {
        ...
        if (item[2] >= Math.floor(Math.random() * 100 + 1)) {
          ...
          await getItem('item', inbattle[i].id, item[0], item[1]);
        }
      }
      if (itemgetlog.length) itemlog = itemlog.concat(itemgetlog);

      await control.write('player', inbattle[i].id, player);
      await control.write('inbattle', inbattle[i].id, {
        channel: '0',
      });
    }
  }
  ...
  if (itemlog.length) {
    let items = itemlog
      .filter((l, i, s) => s.findIndex((x) => x[0] == l[0]) === i)
      .map((m) => m[0]);
    console.log(itemlog);
  ...
  await control.write(`channel`, message.channel.id, channel);
...
const _channel = async (message, action) => {
  let player = await control.read('player', message.author.id);
  let channel = await control.read('channel', message.channel.id);

  if (!player) {
    await control.replace('player', message.author.id, newdata.player);
    player = await control.read('player', message.author.id);
  }
  if (!channel) {
    await control.replace('channel', message.channel.id, newdata.channel);
    channel = await control.read('channel', message.channel.id);
  }
  ...
  switch (action.do || action) {
    case 'sk':
      escape(async () => {
        ...
        let inbattle = await control.read('inbattle', message.author.id);
        if (inbattle) {
          let inchannel = client.channels.cache.get(inbattle.channel);
        ...
        await control.replace('inbattle', message.author.id, message.channel.id); // 戦闘中チャンネルを設定

        if (action.item) {
          let has = await control.read(
            'item',
            {
        ...
        let attack = (a) => {
          ...
          let damage = Math.round(
            ((a.skill.atk * (a.level * 10 + 50)) / 3 / 25) *
              (Math.floor(Math.random() * 85 + 15) / 100),
          );
        ...
        let round = (s) => {
          ...
          if (s) {
            ...
            let enemyskill = skilldata.find((s) => s.id == selectSkill(enemy.skills));
            res = attack({
              pre: '- ',
              skill: enemyskill,
              level: channel.level,
        ...
        await control.write('channel', message.channel.id, channel);
        await control.write('player', message.author.id, player);
    ...
    case 're':
      escape(async () => {
        ...
        if (inbattle) {
          for (let i = 0; i < inbattle.length; i++) {
            let player2 = await control.read('player', inbattle[i].id);

            player2.hp = Math.floor(Math.sqrt(player2.exp)) * 5 + 50;
            player2.mp = Math.floor(Math.sqrt(player2.exp)) + 5;

            await control.write('player', inbattle[i].id, player2);
            await control.write('inbattle', inbattle[i].id, {
              channel: '0',
            });
          }
    ...
    case 'item':
      escape(async () => {
        ...
        if (use) {
          ...
          let player2 = await control.read('player', target.id);
          if (!player2) {
            commanding.set(message.channel.id, false);
          ...
          await control.write('player', target.id, player2);
          player = await control.read('player', message.author.id);
          ...
          commanding.set(message.channel.id, true);
          let items = (await control.read('item', message.author.id, '*', true)) || [];
          ...
          if (!items.length) {
            ...
            items = items
              .sort(
                (a, b) =>
                  itemdata.find((t) => a.item == t.id).pos -
                  itemdata.find((t) => b.item == t.id).pos,
              )
              .map((i) => {
